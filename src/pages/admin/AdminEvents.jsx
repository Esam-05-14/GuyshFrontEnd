...
// import { getEvents, deleteEvent, createEvent, updateEvent } from "../../api/events";
import { getEvents, updataEventId_Admin, createEvent_Admin, deleteEvent_Admin } from "../../services/authService";
...
const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingEvent) {
        // UPDATE
        const updatedEvent = await updataEventId_Admin(editingEvent.id, payload);
        ...
        // CREATE
        const newEvent = await createEvent_Admin(payload);
        setEvents([newEvent, ...events]);
        alert(t("Event created successfully!"));
      }
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                <label className={`block text-sm font-semibold text-gray-700 mb-2 ${isRTL ? "text-right" : ""}`}>
                  {t("Event Image")} <span className="text-gray-500 text-xs">({t("Optional")})</span>
                </label>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              {viewingEvent.image && (
                <div className="rounded-lg overflow-hidden">
                  <img 
                    ...
                    alt={viewingEvent.translations?.[i18n.language]?.title || ""} 
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-[#193042] mb-4">
                  {viewingEvent.translations?.[i18n.language]?.title || viewingEvent.translations?.en?.title || ""}
                </h3>
                
                <p className="text-gray-700 leading-relaxed mb-6">
                  {viewingEvent.translations?.[i18n.language]?.content || viewingEvent.translations?.en?.content || ""}
                </p>
